...
window.drawCards = function(deckType) {
  ...
  if (deck.length === 0) {
    if (GameState.Decks.getDiscardPile(deckType).length === 0) {
      console.warn(`${deckType === 'main' ? 'Main' : 'Alt'} deck and discard pile are empty.`);
      return null; // Nothing to shuffle
    }
    console.warn(`${deckType === 'main' ? 'Main' : 'Alt'} deck is empty. Shuffle discard pile first.`);
    NotificationService.show(`${deckType === 'main' ? 'Main' : 'Alt'} deck is empty. Shuffle discard pile first.`, 'warning');
    return null;
  }
  ...
  if (emptySlots.length === 0) {
    console.warn('No empty slots in player hands');
    NotificationService.show('No empty slots in player hands', 'info');
  ...
  for (let i = 0; i < cardsToDeal; i++) {
    ...
    console.log(`Dealt card ${card.id} to player ${playerIdx + 1}`);
  }
  ...
  NotificationService.show(`Dealt ${cardsToDeal} card${cardsToDeal !== 1 ? 's' : ''} from ${deckType} deck`, 'success');
};
...
window.shuffleDiscardIntoDeck = function(deckType) {
  ...
  const discardPile = GameState.Decks.getDiscardPile(deckType);
  if (discardPile.length === 0) {
    console.warn(`${deckType === 'main' ? 'Main' : 'Alt'} discard pile is empty`);
    if (deckType === 'main'){
      NotificationService.show('Main discard pile is empty', 'warning');
    }
    else{
      NotificationService.show('Alt discard pile is empty', 'warning');
    }

    return;
  }
  
  GameState.Decks.shuffleDiscardIntoDeck(deckType);
  ...
  NotificationService.show(`Shuffled ${deckType} discard pile into deck`, 'success');
};
